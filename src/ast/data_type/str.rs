...
pub fn obj_str(x: String) -> PyObject {
    ...
    let mut method_vec:Vec<(String, PyObjBehaviors)> = build_method!(name:name.clone();param:vec![]);
    method_vec.append(&mut vec![
        build_rust_method(name.clone(), String::from("__len__"), vec![]),
    ]);
    build_method!(
        name: name;
...
pub fn str_behaviour(method: String, args: HashMapAttr) -> PyResult {
    ...
    match method.clone().as_str() {
        "__len__" => {
            match obj_x{
                DataType::Str(x) => {
                    return PyResult::Some(obj_int(x.len() as i64))
                },
                _ => {}
            }
        },
