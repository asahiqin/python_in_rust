...
define_builtin_function!(pattern:{
    define_builtin_function!(func:test;value:"test".to_string())
});
...
type HashMapFunction = HashMap<String,Box<dyn Fn(Namespace,&mut PyNamespace,Vec<DataType>) -> PyResult>>;
impl PyFunction{
    pub fn run(&mut self,vec: Vec<PyObjAttr>,namespace: Namespace,env:&mut PyNamespace) -> Result<PyResult,ErrorType>{
        ...
        let mut result:PyResult = BuiltInFunction{method:self.run_default.clone()}.exec(env,namespace.clone(),data_type_vec);
